...
export class PacientesCitasComponent implements OnInit {
  ...
  ngOnInit(): void{
    ...
    this.pacienteCodigo=Number(this.dataCodigo);
    this.onDataTable();
  }
  ...
  onDeleteCita(select:any):void{
    Swal.fire({
      title: "¿Esta seguro?",
      text: "Este elemento sera cancelado",
      icon: 'warning',
      ...
      if(result.value){
        this.citaService.deleteCita(select.ctCodigo).subscribe(res => {
          if(res){
            Swal.fire("Cancelado",'Se cancelo la cita de manera exitosa','success')
            this.clear();
            this.onDataTable();
          } else {
            alert('Error!')
